...
in
rec {
  hledgerLib = genAttrs ["ghc704" "ghc722" "ghc742" "ghc763" "ghcHEAD"] (ghcVer: genAttrs supportedPlatforms (system:
    let
  ...
  hledger = genAttrs ["ghc704" "ghc722" "ghc742" "ghc763"] (ghcVer: genAttrs supportedPlatforms (system:
    let
      pkgs = import <nixpkgs> { inherit system; };
      haskellPackages = pkgs.lib.getAttrFromPath ["haskellPackages_${ghcVer}"] pkgs;
  ...
  hledgerInterest = genAttrs ["ghc704" "ghc722" "ghc742" "ghc763"] (ghcVer: genAttrs supportedPlatforms (system:
    let
      pkgs = import <nixpkgs> { inherit system; };
      haskellPackages = pkgs.lib.getAttrFromPath ["haskellPackages_${ghcVer}"] pkgs;
      myHledgerLib = pkgs.lib.getAttrFromPath [ghcVer system] hledgerLib;
